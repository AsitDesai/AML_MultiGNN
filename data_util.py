...
from torch_geometric.data import Data, HeteroData  # Importing PyTorch Geometric data structures
from torch_geometric.typing import OptTensor  # Importing optional tensor type
import numpy as np  # Importing NumPy for numerical operations

def to_adj_nodes_with_times(data):
    """
    Converts a graph data object to adjacency lists with timestamps for each node.

    Args:
        data (Data or HeteroData): Input graph data.

    Returns:
        tuple: Two dictionaries containing adjacency lists for incoming and outgoing nodes with timestamps.
    """
...
def to_adj_edges_with_times(data):
    """
    Converts a graph data object to adjacency lists with timestamps for each edge.

    Args:
        data (Data or HeteroData): Input graph data.

    Returns:
        tuple: Two dictionaries containing adjacency lists for incoming and outgoing edges with timestamps.
    """
    num_nodes = data.num_nodes
    timestamps = torch.zeros((data.edge_index.shape[1], 1)) if data.timestamps is None else data.timestamps.reshape((-1,1))
...
def ports(edge_index, adj_list):
    ...
    Args:
        ...
        adj_list (dict): Adjacency list with timestamps.

    Returns:
        Tensor: Port numbers for each edge.
    """
...
def time_deltas(data, adj_edges_list):
    ...
    Args:
        data (Data or HeteroData): Input graph data.
        adj_edges_list (dict): Adjacency list with edge indices and timestamps.

    Returns:
        Tensor: Time deltas for each edge.
    """
...
def z_norm(data):
    """
    Normalizes the input tensor data to have zero mean and unit variance.
...
def create_hetero_obj(x, y, edge_index, edge_attr, timestamps, args):
    """
    Creates a heterogeneous graph object for reverse message passing.

    Args:
        x (Tensor): Node feature tensor.
        y (Tensor): Edge labels tensor.
        edge_index (Tensor): Edge index tensor.
        edge_attr (Tensor): Edge attribute tensor.
        timestamps (Tensor): Edge timestamp tensor.
        args (Namespace): Additional arguments.

    Returns:
        HeteroGraphData: Created heterogeneous graph object.
    """
